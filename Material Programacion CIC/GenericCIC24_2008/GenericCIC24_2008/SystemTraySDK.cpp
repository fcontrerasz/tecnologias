...
void CSystemTray::SetCheckedItems(UINT *mItems, BOOL *sItems, int size)
{
	CheckedItemsSize = size; 
	CheckedItems = new UINT[CheckedItemsSize];
	CheckedItemsState = new BOOL[CheckedItemsSize];

	int i=0;

	while(i<size)
	{
		CheckedItems[i] = mItems[i];
		CheckedItemsState[i] = sItems[i];
		i++;
	}

}

BOOL CSystemTray::SetChecked(UINT uiCmd, BOOL stCmd, BOOL stCheck)
{

	int i = 0;

	while (i < CheckedItemsSize)
	{
		if (uiCmd==CheckedItems[i])
		{
			if (!stCmd)
			{
				if (CheckedItemsState[i]) CheckedItemsState[i] = FALSE;
				else CheckedItemsState[i] = TRUE;
			}
			else CheckedItemsState[i] = stCheck;
			break;
		}
		i++;
	}
...
LRESULT CSystemTray::OnTrayNotification(UINT wParam, LONG lParam) 
{
    ...
    {    
		...
		while (i < CheckedItemsSize)
		{
			if (CheckedItemsState[i]) ::CheckMenuItem(hSubMenu,CheckedItems[i],MF_CHECKED | MF_BYCOMMAND);
			else ::CheckMenuItem(hSubMenu,CheckedItems[i],MF_UNCHECKED | MF_BYCOMMAND);
			i++;
		}
